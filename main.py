    ...
    def calculate_learning(self, date):
        ...
        for avg_score, perf_stud, good_stud, dt in learning:
            if int(dt[0]) == date:
                ...
                if good_stud == '':
                    k = 0
                result = int(avg_score * MULTIPLAYER + n * PER_PERFECT_STUDENT + k * PER_GOOD_STUDENT)
        if result is None:
            return 0
        return result
            ...
            for i in range(len(LEARNING_CRITERIA)):
                ...
                for j in range(len(LEARNING_HEADER)):
                    if j == 0:
                        ...
                        self.data_output.setItem(i, j, QTableWidgetItem(str(res[i])))
                        points = float(res[i]) * MULTIPLAYER
                        total += points
                    elif j == 2 and i > 0:
...
class WorkingWithWordFiles:
    ...
    def make_grades_table(self):
        pass

    def make_achievement_tables(self):
        aspects = self.database.get_data('''SELECT Name FROM Aspects''')
        for i in aspects:
            self.document.add_heading(i, 0)
